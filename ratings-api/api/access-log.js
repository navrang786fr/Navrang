// Returns the admin login/access log (who accessed the admin panel and when).
...
module.exports = async function handler(req, res) {
  ...
  try {
    const log = await readJsonArray(adminRepo, 'access-log.json', adminToken);
    log.sort(function (a, b) { return new Date(b.timestamp) - new Date(a.timestamp); });
    res.status(200).json(log);
  } catch (e) {
    res.status(502).json({ error: 'Could not read access log' });
  }
};
